...
evalValueExpr vExpr =
  case vExpr of
    ...
    ValueExprEntropic quant -> valueFromList [(DafiAssetId, quant)]
...
valueExprTerm = do
    ...
    _ <- spaces
    pure $ case aId of
      DafiAssetId -> ValueExprEntropic q
...
assetId =
    try dafiAssetId
      ...
      <?> "asset ID"
  where
    -- Parse the DAFI asset ID.
    dafiAssetId :: Parser AssetId
    dafiAssetId = string "entropic" $> DafiAssetId

    -- Parse a multi-asset ID.
    nonDafiAssetId :: Parser AssetId
    nonDafiAssetId = do
      polId <- policyId
