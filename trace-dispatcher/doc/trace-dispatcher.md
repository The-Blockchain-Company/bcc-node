...
The __frequency limiter__ itself emits a __suppression sumjen__ message under the following conditions:
...
`forwardingTracer` is intended to be used as a __message__ source for `RTView` and `bcc-logger`.
...
Configuring a __trace-out__ to output human-readable text (and therefore to use the human formatter), produces a presentation of the form `[HOST:NAMESPACE] (SEVERITY.THREADID)`:
    ...
    [deus-x-machina:bcc.general.middle.specific](Info.379)
...
>
> We should consider that we already have a dedicated `bcc-logger` component for file logging.
