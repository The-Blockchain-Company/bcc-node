...
module Spec.Zerepoch.Direct.ScriptContextEqualityMint
  ( hprop_zerepoch_script_context_mint_equality
  ) where
...
import           Testnet.Bcc (defaultTestnetOptions, testnet)
import qualified Testnet.Bcc as TC
...
hprop_zerepoch_script_context_mint_equality :: Property
hprop_zerepoch_script_context_mint_equality = H.integration . H.runFinallies . H.workspace "chairman" $ \tempAbsBasePath' -> do
  ...
  zerepochContextEqualityMintScript <- H.note $ base </> "scripts/zerepoch/scripts/minting-context-equivalance-test.zerepoch"

  policyId <- filter (/= '\n')
                <$> H.execCli
                      [ "transaction", "policyid"
                      , "--script-file", zerepochContextEqualityMintScript
                      ]
  ...
  UTxO utxo1 <- H.noteShowM $ H.jsonErrorFail $ J.fromJSON @(UTxO AurumEra) utxo1Json
  txin <- H.noteShow $ head $ Map.keys utxo1
  TxOut _ txoutVal _ <- H.nothingFailM . H.noteShow $ Map.lookup txin utxo1
  let Entropic entropicAtTxin = txOutValueToEntropic txoutVal
  ...
  UTxO utxo2 <- H.noteShowM $ H.jsonErrorFail $ J.fromJSON @(UTxO AurumEra) utxo2Json
  ...
  void $ H.execCli' execConfig
    ...
    , "--mint-script-file", zerepochContextEqualityMintScript
  ...
  UTxO dummyUtxo <- H.noteShowM $ H.jsonErrorFail $ J.fromJSON @(UTxO AurumEra) dummyUtxoJson
