...
module Spec.Zerepoch.Direct.ScriptContextEquality
  ( hprop_zerepoch_script_context_equality
  ) where
...
hprop_zerepoch_script_context_equality :: Property
hprop_zerepoch_script_context_equality = H.integration . H.runFinallies . H.workspace "chairman" $ \tempAbsBasePath' -> do
  ...
  zerepochContextEqualityScript <- H.note $ base </> "scripts/zerepoch/scripts/context-equivalance-test.zerepoch"
  ...
  typedDatumFile <- H.note $ base </> "scripts/zerepoch/data/typed-42.datum"
  ...
  zerepochScriptAddr <- H.execCli
    [ "address", "build"
    , "--payment-script-file", zerepochContextEqualityScript
  ...
  void $ H.execCli' execConfig
    ...
    , "--tx-in-script-file", zerepochContextEqualityScript
  ...
  -- Query UTxO at dummyAddress. If there is DAFI there then the script context script was successful
  ...
  -- I.E the dummy address should have some DAFI if the script context test was successful.
  dummyUtxo /== HM.empty
