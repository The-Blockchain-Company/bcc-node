...
import           Bcc.Ledger.Address (getRewardAcnt)
import qualified Bcc.Ledger.Sophie.API as L
import qualified Bcc.Ledger.Sophie.Rewards as L
import qualified Bcc.Ledger.Sophie.RewardUpdate as L
...
data Event c
  ...
  | MIREvent SlotNo Entropic L.MIRPot
  | WithdrawalEvent SlotNo Entropic
  | RewardBalanceEvent EpochNo SlotNo Entropic
  | RewardStartEvent EpochNo SlotNo Entropic
...
msg ev = putStrLn (message ev)
  where
    ...
    displayReward (L.Reward t (L.KeyHash kh) love) =
      show t <> "-" <> (tail . init $ show kh) <> "-" <> show (fromSophieEntropic love)
...
main = do
  ...
  let targetCred = target args
      ...
      targetCredAsAPI = fromSophieStakeCredential targetCred
  ...
  !_ <- fmap f $ runExceptT $ foldBlocks
         ...
         (\_env
           ...
           state -> do
             ...
             -- in non-cole eras, get the necessary components of the ledger state
             let (name, sophieState) =
                   case ledgerState of
                     LedgerStateCole _                                     ->
                       ("cole",   Nothing)
                     LedgerStateSophie (Sophie.SophieLedgerState _ ls _) ->
                       ("sophie", Just (L.nesEL ls, L.nesRu ls, getGoSnapshot ls, getBalances ls))
                     LedgerStateEvie (Sophie.SophieLedgerState _ ls _) ->
                       ("evie", Just (L.nesEL ls, L.nesRu ls, getGoSnapshot ls, getBalances ls))
                     LedgerStateJen    (Sophie.SophieLedgerState _ ls _) ->
                       ("jen",    Just (L.nesEL ls, L.nesRu ls, getGoSnapshot ls, getBalances ls))
                     LedgerStateAurum    (Sophie.SophieLedgerState _ ls _) ->
                       ("aurum",    Just (L.nesEL ls, L.nesRu ls, getGoSnapshot ls, getBalances ls))
  ...
  where
    ...
    stakeCredentialFromStakeAddress (StakeAddress _ cred) = fromSophieStakeCredential cred
    ...
    rewardBalance epochLast epochCurrent slot rs t =
      epochEvent epochLast epochCurrent slot rs t
        (\e s v -> RewardBalanceEvent e s (fromSophieEntropic v))
    ...
    rewardStartEvent epochLast epochCurrent slot ss t =
      epochEvent epochLast epochCurrent slot ss t
        (\e s v -> RewardStartEvent e s (fromSophieEntropic v))
