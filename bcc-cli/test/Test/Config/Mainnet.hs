...
import           Bcc.Api (initialLedgerState, renderInitialLedgerStateError)
...
hprop_configMainnetHash = H.propertyOnce $ do
  ...
  result <- H.evalIO $ runExceptT $ initialLedgerState $ projectBase </> "configuration/bcc/mainnet-config.json"
  case result of
    Right (_, _) -> return ()
...
hprop_configMainnetYaml = H.propertyOnce $ do
  projectBase <- H.note =<< H.evalIO . IO.canonicalizePath =<< H.getProjectBase
  yamlResult <- H.evalIO . Y.decodeFileEither $ projectBase </> "configuration/bcc/mainnet-config.yaml"
  yaml :: J.Value <- case yamlResult of
    Right v -> return v
    Left e -> H.failWithCustom GHC.callStack Nothing (Y.prettyPrintParseException e)
  jsonResult <- H.evalIO . J.eitherDecodeFileStrict $ projectBase </> "configuration/bcc/mainnet-config.json"
